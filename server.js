...
app.post("/api/attendance/mark", requireTeacher, async (req, res) => {
  ...
  try {
   ...
   const existing = data.attendance[classId][today];

if (existing) {
  // Allow only same teacher to update
  if (existing.teacherId !== req.session.teacher.id) {
    return res.status(403).json({
      error: "Attendance already submitted by another teacher today"
    });
  }

  // If same teacher, overwrite allowed
}
  ...
  try {
    ...
    students.forEach((student, index) => {
      ...
      doc
        ...
        .fontSize(8)
        .text(
          "Behind Garko Motor Park, Opp. Tasidi Filling Station",
          START_X,
          startY + 68,
          { width: WIDTH, align: "center" }
        )
...
doc
  ...
  .text(`Student Name: ${student.name}`, START_X, bodyStartY + lineGap)
  .text(`Class: ${classId}`, START_X, bodyStartY + lineGap * 2)
...
app.get("/api/admin/student-stats-pdf", (req, res) => {
  try {
    ...
    function drawHeader() {
      ...
      const schoolName = "ASSALAM INTERNATIONAL ACADEMIC SCHOOL GARKO";
