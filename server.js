...
const QUESTION_IMAGE_DIR = path.join(__dirname, 'public', 'uploads');
...
function ensureDir(dirPath) {
  try {
    ...
    if (fs.existsSync(dirPath)) {
      // If it exists but is a file, remove it first
      if (!fs.lstatSync(dirPath).isDirectory()) {
        fs.unlinkSync(dirPath); // remove the file
...
[CSV_UPLOAD_DIR, QUESTION_IMAGE_DIR].forEach(ensureDir);
